...
* ⚠️ **Robust Error Types** - Specific error classes for HTTP, Network, and Parsing issues
...
`endpoint-fetcher` allows you to define the structure of expected error responses using a third generic parameter. When a request fails, the error object will contain a typed `error` property representing the response body.

### Defining Error Types

| **Type**    | **Generic Signature**      | **Description**                           |
| ----------------- | -------------------------------- | ----------------------------------------------- |
| **Default** | `get<TInput, TOutput>`         | The error body defaults to `any`.             |
| **Custom**  | `get<TInput, TOutput, TError>` | The error body is strictly typed to `TError`. |
...
```typescript
import { get, post } from 'endpoint-fetcher';
...
const getUser = get<{ id: number }, User, SimpleError>(
  input => `/users/${input.id}`
);

// 2. Detailed Validation Error
...
When an error occurs, the `error` property on the caught exception is automatically typed based on your endpoint definition.

**TypeScript**

```typescript
try {
  await api.createUser({ name: 'John', email: 'invalid-email' });
} catch (error) {
  // error.error is strictly typed as ValidationError
...
`endpoint-fetcher` can be extended via plugins. A common example is the official caching plugin, which adds a `plugins` array to your configuration.

### Example: `@endpoint-fetcher/cache`
...
**Usage**
To use a plugin, import it and add it to the `plugins` list in `createApiClient`. Use the `CachingWrapper` type on endpoints you want to cache.
...
const api = createApiClient({
  users: {
    endpoints: {
      list: get<void, CachingWrapper<User[]>>('/users'),
    }
  }
}, {
  baseUrl: 'https://api.example.com',
  plugins: [
    cache({ ttl: 300 }) // Plugin initialized here
  ]
});

// The response now includes cache metadata and actions
const result = await api.users.list();
console.log(result.data);    // The actual User[] data
console.log(result.isStale); // Metadata provided by the plugin
await result.refresh();      // Action provided by the plugin
```

For more details on caching strategies and storage adapters, see the [@endpoint-fetcher/cache documentation](https://github.com/lorenzo-vecchio/endpoint-fetcher-cache).
...
* `config`: Client configuration
  ...
  * `fetch?`: Optional custom fetch instance (useful for plugins)
